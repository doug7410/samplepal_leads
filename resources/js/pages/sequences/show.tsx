...
import { type BreadcrumbItem, type Sequence, type SequenceStats, type Contact, type Company } from '@/types';
...
export default function SequenceShow({ sequence, statistics }: SequenceShowProps) {
  ...
  const handleAddContacts = () => {
    ...
    router.post(route('sequences.add-contacts', { sequence: sequence.id }), {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card className="p-5">
            ...
            {statistics.exited > 0 ? (
              ...
              <p className="text-gray-500 text-sm">No contacts have exited yet.</p>
            )}
          </Card>
        </div>
        ...
        <Card className="p-5">
          ...
          {sequence.sequence_contacts && sequence.sequence_contacts.length > 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left py-2 px-3">Current Step</th>
                    <th className="text-left py-2 px-3">Next Send</th>
                ...
                <tbody>
                  {sequence.sequence_contacts.map((sc) => (
                    <tr key={sc.id} className="border-b">
                      ...
                      <td className="py-2 px-3">{sc.contact?.company?.company_name}</td>
                      <td className="py-2 px-3">
                        <Badge className={contactStatusBadge[sc.status]?.color || 'bg-gray-100'}>
                          {contactStatusBadge[sc.status]?.label || sc.status}
                        </Badge>
                        {sc.exit_reason && (
                          <span className="text-xs text-gray-500 ml-2">({sc.exit_reason})</span>
                        )}
                      </td>
                      <td className="py-2 px-3">
                        Step {sc.current_step + 1}
                      </td>
                      <td className="py-2 px-3">
                        {sc.next_send_at ? new Date(sc.next_send_at).toLocaleString() : '-'}
                      </td>
                      <td className="py-2 px-3">
                        {sc.status === 'active' && (
                          <Button
            ...
            <p className="text-gray-500 text-sm text-center py-8">
              No contacts enrolled yet. Add contacts to start the sequence.
            </p>
          )}
        </Card>
          ...
          <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="py-4">
              ...
              <p className="text-sm text-gray-500">
                This feature will be enhanced in a future update to show a contact picker here.
              </p>
            </div>
            <DialogFooter>
