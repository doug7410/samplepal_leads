...
import { type BreadcrumbItem, type Company, type Contact, type Sequence } from '@/types';
...
export default function SequenceEdit({ sequence, companies, contacts }: SequenceEditProps) {
  const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Dashboard', href: '/dashboard' },
    { title: 'Sequences', href: '/sequences' },
    { title: sequence.name, href: `/sequences/${sequence.id}` },
    { title: 'Edit', href: `/sequences/${sequence.id}/edit` },
  ];

  const initialSteps: StepData[] = sequence.steps?.map(step => ({
    name: step.name,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    router.put(route('sequences.update', { sequence: sequence.id }), {
      name,
      description,
      steps,
      entry_filter: sequence.entry_filter,
    } as any, {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" asChild className="mr-2">
              <Link href={route('sequences.show', { sequence: sequence.id })}>
                <ChevronLeft size={16} />
